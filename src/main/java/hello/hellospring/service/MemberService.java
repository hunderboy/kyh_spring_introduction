...
/**
 ...
 // todo : MemberService 를 테스트 하기 위해서
 // todo : 'MemberService' 에 커서를 두고, 단축키 : Commend + shift + T 하면, Create New Test 를 빠르고 손쉽게 생성할수 있다.
 */
...
public class MemberService {
    ...
    @Autowired
    public MemberService(MemberRepository memberRepository) { // 생성자를 이용하여, 외부에서 MemberRepository 받을수 있게끔 설정 = 의존성 주입
    ...
    private void validateDuplicateMember(Member member) {
         ...
         * todo : Mac 에서는 Command + option + M => 드래그 한 부분 함수로 추출
         */
        memberRepository.findByName(member.getName())
                .ifPresent(m -> { // ifPresent = null 값이 아닌 어떤 값이 존재하면 처리 = return 값이 Optional 이라서 가능
                    throw new IllegalStateException("이미 존재하는 회원입니다.");
                });
    }
